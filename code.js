...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function executeCommand(command) {
    ...
    switch (command) {
      case 'ls': output.innerHTML += `<div><span style="color:blue">discord</span><span style="color:blue"> steam</span><span style="color:blue"> twitter</span></div>`;
        break;
      ...
      case 'cat discord-server': output.innerHTML += `<div>${prompt}${'<a href="https://discord.gg/dTrXDXQdGr" target="_blank">server</a>'}</div>`;
        break;

      case 'cat instagram': output.innerHTML += `<div>${prompt}${'<a href="https://www.instagram.com/dxmsch02/" target="_blank">dxmsch02</a>'}</div>`;
        break;
      ...
      case 'cat': output.innerHTML += `<div>${prompt}${'cat [file]'}</div>`;
        break;

      case 'rm -rf': navigateToAnotherPage();
        break;
      ...
      case 'cat X': output.innerHTML += `<div>${prompt}${'<a href="https://twitter.com/swaggyiroh" target="_blank">swaggyiroh</a>'}</div>`;
        break;

      default: ;
        count += 1;
        if (count == 5) {
          output.innerHTML += `<div>${prompt}${'try using "ls"'}</div>`;
          count = 0;
        } else { output.innerHTML += `<div>${prompt}${command + ': command not found!'}</div>` }
        break;
