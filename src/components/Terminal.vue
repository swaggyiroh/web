...
<template>
  <div class="flex flex-col justify-center items-center w-screen">
    <div class="w-full max-w-6xl mx-auto bg-black text-white font-vt323 p-4 rounded-lg text-xl">
      ...
      <div class="h-96 overflow-auto select-text" ref="terminalOutput">
        <div v-for="(line, index) in output" :key="index" v-html="line"></div>
        <div class="flex items-center">
          <span class="text-white"><span class="text-green-500">visitor@github-pages</span>:<span
              class="text-blue-700">{{ currentDir }} </span>$</span>
          <input v-model="command" @keydown.enter="executeCommand" ref="terminalInputRef"
